...
Om modelinzichten, in de ![ Modellen ](/help/assets/icons/FileData.svg) **[!UICONTROL Models]** interface in [!DNL Mix Modeler] te bekijken:

1. Van de **[!UICONTROL Models]** lijst, selecteer de naam van een model dat a **[!UICONTROL Last run status]** van ![ StatusGreen ](/help/assets/icons/StatusGreen.svg) **[!UICONTROL Success]** heeft.
...
* [Kanaalanalyse](#channel-analysis)
* [ Factoren ](#factors) [!BADGE  bèta ]
...
## Modelinzichten {#model-insights-section}

Het **[!UICONTROL Model insights]** lusje toont visualisaties voor [ Bijdrage door datum en basismedia ](#contribution-by-date-and-base-media), [ Bijdrage door kanaal ](#contribution-by-channel), [ de prestatiesoverzicht van de Marketing ](#marketing-performance-summary), en [ Marginale reactiecurven ](#marginal-response-curves). Het lusje verstrekt ook a [ de onderbreking van het aanraakpunt ](#touchpoint-breakdown) lijst.
...
### Overzicht van marketingprestaties {#marketing-performance-summary}

>[!CONTEXTUALHELP]
>id="models_insights_undefinedchannels"
>title="Ongedefinieerde kanalen"
>abstract="Ongedefinieerde kanalen worden opgenomen, maar hebben geen toegewezen omzettingen."
...
Ongedefinieerde kanalen worden in de visualisatie opgenomen, maar hebben geen toegewezen conversies.


### Marginale responscurven

Het lijndiagram visualiseert en vergelijkt de marginale opbrengsten die door de investering in uw marketing kanalen worden geproduceerd.  En identificeert het huidige bestedingspunt en het marginale break-even punt (waar uw stijgende terugkeer minder dan uw stijgende uitgaven is). Hierdoor helpt deze visualisatie u te begrijpen wanneer uw marketinginvestering minder effect begint te sorteren.

De curve, het huidige bestedingspunt, het marginale break-even punt en de bijbehorende waarden worden berekend op basis van het geselecteerde gegevensbereik en het kanaal dat u hebt geselecteerd.
...
Het **[!UICONTROL Channel analysis]** lusje toont visualisatie voor **[!UICONTROL Channel synergies]** en **[adstock van het Kanaal]**.


### Kanaalsynergieën

Met behulp van de Kanaalsynergie-visualisatie kunt u bepalen hoe marketingkanalen interageren om multiplicatieve effecten te creëren, die verder gaan dan hun individuele bijdragen.

De heatmap-matrix biedt een visuele weergave van de synergie-waarden tussen paren uitgavenkanalen. Deze matrix helpt marketers te begrijpen hoe kanalen op elkaar inwerken om de prestaties te stimuleren. Voor elk model worden de synergie-waarden genormaliseerd van 0 tot 10. Deze waarden kwantificeren de *volgende dollarsynergie*, die schat hoe effectief twee kanalen samenwerken wanneer elk één extra dollar van uitgaven op de huidige niveaus ontvangt.

Dit kader van de volgende dollar biedt een realistische maatstaf voor relatieve synergie-effecten, aangezien het kader de werkelijke uitgavenomstandigheden in de opleidingsgegevens verwerkt en als zodanig beter geïnformeerde optimalisatiebesluiten mogelijk maakt.


>[!BEGINTABS]

>[!TAB  toon minder synergieën ]

![ het kanaalsynergieën van het Plan ](/help/assets/model-channel-synergies-less.png)

>[!TAB  toon alle synergieën ]

![ het kanaalsynergieën van het Plan ](/help/assets/model-channel-synergies-all.png)

>[!ENDTABS]


* Selecteer **[!UICONTROL Show all]** om alle synergieën weer te geven.

* Selecteer **[!UICONTROL Show less]** als u minder synergieën wilt selecteren

* Houd de muisaanwijzer boven een cel in de visualisatie om details van een synergie te tonen.

* Om een Csv- dossier te downloaden dat de matrijs vertegenwoordigt, uitgezochte ![ Download ](/help/assets/icons/Download.svg) **[!UICONTROL Download]**.

>[!NOTE]
>
>Als het tabblad **[!UICONTROL Channel synergy]** niet zichtbaar is voor een bestaand model, moet u ervoor zorgen dat u het model opnieuw traint om de functionaliteit en visualisatie in te schakelen.


### Kanaalpapier

De visualisaties van het kanaaladstock tonen voor elk gevormd kanaal een visualisatie. De visualisaties helpen u om te begrijpen hoe het effect van marketing uitgaven voor elk kanaal in tijd en voorbij de aanvankelijke blootstelling voortduurt en vervalt. De visualisatie verstrekt een realistische mening van kanaaloverdracht door de opname van tijdelijke dynamiek die van het model wordt geleerd.

Elke individuele grafiek vertegenwoordigt één enkel marketing kanaal en toont zijn adstock kromme. Die curve modelleert hoe het effect van een blootstellingseenheid zich in toekomstige perioden voordoet. De curve illustreert de snelheid waarmee het effect op conversie vermindert en helpt marketers te vergelijken hoe lang verschillende kanalen de prestaties na de initiële investering blijven beïnvloeden.

Kanalen met langzamere verval curven (langere staarten) geven een aanhoudend effect in de loop der tijd aan. Kanalen met steile drop-outs weerspiegelen meer directe, kortstondige effecten. De maximumterugkijkperiode die voor dat kanaal wordt gevormd bepaalt de lengte van elke kromme.

>[!BEGINTABS]

>[!TAB  toon al adstock ]

![ het kanaalsynergieën van het Plan ](/help/assets/model-channel-adstock-all.png)

>[!TAB  toon geselecteerd adstock ]

![ het kanaalsynergieën van het Plan ](/help/assets/model-channel-adstock-selected.png)

>[!ENDTABS]

* Selecteer een optie in het vervolgkeuzemenu **[!UICONTROL Channel]** om de visuele oriëntaties voor **[!UICONTROL All channels]** of een aangepaste visualisatie voor een afzonderlijk kanaal (bijvoorbeeld **[!UICONTROL Paid Social Facebook]** ) weer te geven.
...
>title="Uitsplitsing naar factor-bijdrage"
>abstract="De uitsplitsing naar factor-bijdrage geeft het aandeel van de basisomzettingen weer dat kan worden toegeschreven aan de verschillende factoren die in het model zijn opgenomen.<br/><br/> de Zuivere basis vertegenwoordigt de onderliggende omzettingen die onafhankelijk van de marketing touchpoints en factoren inbegrepen in het model voorkomen. Hieronder vallen conversies die worden gestuurd door het eigen vermogen van merken, herhaalde aankopen, de biologische vraag en markttendensen op lange termijn en seizoensgebondenheid."
...
>title="Uitsplitsing naar kanaal"
>abstract="**[!UICONTROL Breakdown by channel]** is een verdeling door het kanaaltype voor de bepaalde aanraakpunten, die op het de gebeurtenisschema van de klantenervaring wordt gebaseerd. Selecteer ![ Meer ](https://spectrum.adobe.com/static/icons/workflow_18/Smock_MoreSmallList_18_N.svg) en **[!UICONTROL Breakdown by touchpoint]** om een onderbreking door touchpoint te tonen."


>[!CONTEXTUALHELP]
>id="models_attribution_breakdownbytouchpointposition"
>title="Uitsplitsing naar positie aanraakpunt"
>abstract="Deze visualisatie laat een uitsplitsing zien van toegewezen omzettingen naar positie van het aanraakpunt en aanraakpunt over alle omzettingspaden. De visualisatie vergelijkt of een aanraakpunt op een positie beter bijdraagt dan op resterende posities en andere aanraakpunten op een willekeurige positie."
...
* Gebaseerd op het geselecteerde model in [!DNL Mix Modeler]:
   * Algorithmic - Invloed
...
Zie [ Multi-aanraking attributie ](../get-started/about.md#multi-touch-attribution) voor een inleiding op het multi-aanraak attributievermogen in [!DNL Mix Modeler].
...
De granulaire gebeurtenisscores voor Mix Modeler-multitouch-kenmerk worden uitgelijnd op de algemene scores en ROI&#39;s voor [!DNL Mix Modeler] . Deze scores worden ook beschikbaar gesteld als datasets in Experience Platform.
...
In het dialoogvenster uitgevouwen **[!UICONTROL Top campaigns]** wordt dezelfde tabel weergegeven met optelkolommen voor:
...
>title="Metrische modelgegevens"
>abstract="Toont een overzicht van veelvoudige model opleidings passende metriek."
...
>title="Efficiëntiemaatregel"
>abstract="De efficiëntiemaatstaf die door het algoritmische attributiemodel wordt gegenereerd, geeft het relatieve belang aan van een aanraakpunt voor de conversie onafhankelijk van het aanraakpuntvolume. Deze maatregel heeft een schaal van 1 tot 5. Het hogere aanraakpuntvolume garandeert geen hogere efficiëntiemaatstaf."


>[!CONTEXTUALHELP]
>id="models_diagnostics_totalvolume"
>title="Totaal volume"
>abstract="Het totale volume is het totale aantal keren dat een gebruiker een aanraakpunt aanraakt. En bevat alle aanraakpunten die worden weergegeven op een pad dat bereikt en paden die niet resulteren in conversie."


>[!CONTEXTUALHELP]
>id="models_diagnostics_modeldateinfo"
>title="Modeldatum vanaf"
>abstract="De gegevens voor deze tabel worden alleen voor specifieke tijdsperioden gegenereerd.  De **[!UICONTROL As of]** -datum geeft aan wanneer de gegevens zijn gegenereerd en is gebaseerd op gegevens van de opgegeven datum tot een jaar terug."
...
* **[!UICONTROL Model training fit metrics]** tabel die voor elke metrische conversie wordt weergegeven:
   ...
   * **[!UICONTROL Out-of-sample RMSE]**: evalueert percentagefout op onzichtbare gegevens, in evenwicht brengend over en ondervoorspelt. Helpt de generalisatie te beoordelen. [!DNL Mix Modeler] evalueert momenteel het percentage van een fout met behulp van het laatste kwartaal van de trainingsgegevens als een holdout-set. RMSE penalizes grotere fouten meer dan MAPE.
...
>[!AVAILABILITY]
>
>De in deze sectie beschreven functionaliteit bevindt zich in de beperkte testfase van de release en is mogelijk nog niet beschikbaar in uw omgeving. Deze notitie wordt verwijderd wanneer de functionaliteit over het algemeen beschikbaar is. Voor informatie over het [!DNL Mix Modeler] versieproces, zie [ de eigenschapversies van Mix Modeler ](/help/releases/latest.md).
>
...
Een alternatief op uitgaven-gebaseerd kanaal voor deze visualisatie tonen:
...
U kunt als volgt een alternatief op volume gebaseerd kanaal voor deze visualisatie weergeven:
